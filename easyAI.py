...
def play():
    ...
    def display_menu(screen):
        ...
        title_text = font.render("You Selected Easy Mode!", True, WHITE)
    ...
    while not game_over:
        ...
        # If it is AI's turn and the game is not over, select a column at random
        if turn == AI and not game_over:				
            col = random.randint(0, COLUMN_COUNT-1)

            # If the selected column is a valid location, wait a moment and then drop the disc
            if valid_location(board, col):
                ...
                # Switches turn from AI to player
                turn += 1
                turn = turn % 2
